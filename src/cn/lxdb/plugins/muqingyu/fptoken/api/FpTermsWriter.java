...
public class FpTermsWriter {
	...
	public void writeTerms(int maxDoc, ObjectPoolMulti pool, Terms terms, org.apache.lucene.codecs.blocktree.BlockTreeTermsWriter$TermsWriter termsWriter,
		...
		try (FpBitIndexTempDirectory.Session session = FpBitIndexTempDirectory.INS.openSession(field);
				FpBitIndexSegmentStaging bitIndexStaging = new FpBitIndexSegmentStaging(session)) {
			final FpTokenBlockOrchestrator orchestrator = new FpTokenBlockOrchestrator(fpblock_list, termWriter, terms,
					maxDoc, field, pool, termsWriter, debug_list, norms, bitIndexStaging);
