...
class FpBlockInfoSerializationTest {
	...
	void writeto_readfrom_roundTrip() throws Exception {
		...
		expected.docCount = 99;
		expected.fieldInfo = new org.apache.lucene.util.BytesRef("col");
		...
		try (IndexInput in = dir.openInput("meta", IOContext.DEFAULT)) {
			...
			assertEquals(expected.docCount, actual.docCount);
			assertEquals("col", actual.fieldInfo.utf8ToString());
		}
	}
}
